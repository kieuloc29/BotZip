...
"https://i.imgur.com/QYGOZuK.jpg",
  ...
  var money = data.money
      if(money < 100000) api.sendMessage("Đéo đủ tiền đòi xem cái lồn, kiếm đủ 10000 đô rồi xem, oke ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 100000})
   var callback = () => api.sendMessage({body:`Mấy ba khoái lắm😼\n» Số dư: -100000 đô «`,attachment: fs.createReadStream(__dirname + "/cache/5.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.jpg")); 
